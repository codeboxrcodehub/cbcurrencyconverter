...
if ( ! class_exists( 'CBCurrencyconverterSetting' ) ):
	...
	class CBCurrencyconverterSetting {
		...
		function callback_heading( $args ) {
			$plus_svg  = cbcurrencyconverter_load_svg( 'icon_plus' );
			$minus_svg = cbcurrencyconverter_load_svg( 'icon_minus' );
			...
			$html = '<h3 class="setting_heading"><span class="setting_heading_title">' . esc_html( $args['name'] ) . '</span><a title="' . esc_attr__( 'Click to show hide',
					'cbcurrencyconverter' ) . '" class="setting_heading_toggle button outline primary icon-only icon-inline" href="#"><i class="cbx-icon cbx-icon-img setting_heading_toggle_plus">' . $plus_svg . '</i><i class="cbx-icon cbx-icon-img setting_heading_toggle_minus">' . $minus_svg . '</i></a></h3>';
		...
		function callback_custom_rates( $args ) {
			...
			foreach ( $from_curs as $from_cur ) {
				...
				echo '<div class="custom_rates_content custom_rates_content_' . esc_attr( $from_cur ) . '  ' . esc_html( $active_class ) . '" style="' . esc_attr($hide_style) . '">';

				/* translators: %s: curency name */
				echo '<p class="custom_rate_heading">' . sprintf( esc_html__( 'Custom rates based on %s', 'cbcurrencyconverter' ), esc_attr($from_cur) ) . '</p>';
				...
				if ( ! empty( $all_currencies ) ) {
					foreach ( $all_currencies as $code => $currency ) {
						...
						echo '<p class="custom_rate_name">' . esc_attr($currency) . '-' . esc_attr($code) . '</p>';
						echo '<input type="number" style="height: 30px;" class="regular-text" id="' . esc_attr($args['section']) . '_custom_rates_' . esc_attr($from_cur) . '_' . esc_attr($code) . '" name="' . esc_attr($args['section']) . '[custom_rates][' . esc_attr($from_cur) . '-' . esc_attr($code) . ']" value="' . esc_attr($value) . '" step="0.000000001"/>';
						echo '<a data-busy="0" type="button" data-from="' . esc_attr($from_cur) . '" data-to="' . esc_attr($code) . '" title="' . esc_attr__( 'Auto fill from Api Source', 'cbcurrencyconverter' ) . '" class="button outline primary auto_fill_customrate ld-ext-right">' . esc_attr__( 'Auto Fill via Api', 'cbcurrencyconverter' ) . '<span class="ld ld-ring ld-spin"></span></a>';
						echo '</div>';
					}
				}
		...
		function callback_woocomemrce_check( $args ) {
			...
			$html = '<div class="plugin_check_wrap ' . esc_attr( $woo_active_class ) . '">' . esc_html($woo_active_text) . '</div>';
			echo $html; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		...
		function show_navigation() {
			...
			foreach ( $this->settings_sections as $tab ) {
				...
				$html        .= sprintf( '<a data-tabid="' . esc_attr($tab['id']) . '" href="#%1$s" class="%3$s" id="%1$s-tab">%2$s</a>', $tab['id'], $tab['title'], $active_class );
					...
					foreach ( $this->settings_sections as $form ) {
                        ...
                        <div id="<?php echo esc_attr($form['id']); ?>" class="global_setting_group" style="<?php echo esc_attr($display_style); ?>">
                            <form method="post" action="options.php" class="cbcurrencyconverter_setting_form">
								...
								do_action( 'cbcurrencyconverter_setting_form_top_' . esc_attr($form['id']), $form );

								settings_fields( $form['id'] );
								do_settings_sections( $form['id'] );

								do_action( 'cbcurrencyconverter_setting_form_bottom_' . esc_attr($form['id']), $form );
                                ...
                                <div class="global_setting_submit_buttons_wrap">
									...
									<?php do_action( 'cbcurrencyconverter_setting_submit_buttons_start', esc_attr($form['id']) ); ?>
									<?php submit_button( esc_html__( 'Save Settings', 'cbcurrencyconverter' ), 'button primary submit_setting', 'submit', true, [ 'id' => 'submit_' . esc_attr( $form['id'] ) ] ); ?>
									<?php do_action( 'cbcurrencyconverter_setting_submit_buttons_end', esc_attr($form['id']) ); ?>
                                </div>
                            </form>
                        </div>
						<?php
